...
(defun run-test-seeds-experiment (exp &rest args)
  ...
  (let ((state-key (fill-adt (make-state-key)
    ...
    (rl-set-state exp state-key)
    (check exp #'rl-equalp state-key (rl-get-state exp))
    (rl-set-random-seed exp random-seed-key)
    (check exp #'rl-equalp random-seed-key (rl-get-random-seed exp))
    (rl-set-state exp empty-state-key)
