...
\begin{itemize}
\item[] cl-ppcre (\selfref{http://www.weitz.de/cl-ppcre/})
\end{itemize}
...
\lispprompt{(defmethod env-get-state ((env my-env)) \ldots)} \\
\lispprompt{(defmethod env-set-state ((env my-env) state-key) \ldots)} \\
\lispprompt{(defmethod env-get-random-seed ((env my-env)) \ldots)} \\
\lispprompt{(defmethod env-set-random-seed ((env my-env) random-seed-key) \ldots)} \\
...
\prompttext{rl-init}, \prompttext{rl-start}, \prompttext{rl-step},
\prompttext{rl-cleanup}, \prompttext{rl-return}, \prompttext{rl-num-steps},
\prompttext{rl-num-episodes}, \\
\prompttext{rl-episode}, \prompttext{rl-get-state}, \prompttext{rl-set-state},
\prompttext{rl-get-random-seed}, \prompttext{rl-set-random-seed}, \\
\prompttext{rl-agent-message}, \prompttext{rl-env-message}.

A detailed description of these functions can be obtained this way. \\
\lispprompt{(documentation \#'<function name> 'function)}

Do not forget to call the \prompttext{rl-cleanup} function at the end of your
...
It can parse the task specification string into a \prompttext{task-spec}
structure, e.g. \\
\lispprompt{(parse-task-spec "2:e:1\_[i]\_[0,107]:1\_[i]\_[0,3]:[-10,10]")}

Because the language is under construction, the parser can change in the
near future.
...
\prompttext{RL-Glue Lisp Experiment Codec Version 1.0-RC2, Build 345 \\
\mbox{~~~~~~~~}Connecting to 127.0.0.1:4096 .~ok \\
Task spec was:~2:e:1\_[i]\_[0,107]:1\_[i]\_[0,3]:[-10,10] \\
.................................................. \\
----------------------------------------------- \\
